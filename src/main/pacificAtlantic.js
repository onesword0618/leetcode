...
module.exports = (matrix) => {
  ...
  while (queue.length) {
    const p = queue.shift();
    DIRS.map((d) =>
      addPacific(matrix, p[0], p[1], p[0] + d[0], p[1] + d[1], queue)
    );
  }
  ...
  while (queue.length) {
    const a = queue.shift();
    DIRS.map((d) =>
      addAtlantic(matrix, a[0], a[1], a[0] + d[0], a[1] + d[1], queue, res)
    );
  }
...
const addPacific = (matrix, or, oc, r, c, queue) => {
  if (
    or < 0 ||
    (matrix[r] && matrix[r][c] >= 0 && matrix[r][c] >= abs(matrix[or][oc]))
  ) {
...
const addAtlantic = (matrix, or, oc, r, c, queue, res) => {
  if (
    or < 0 ||
    (matrix[r] &&
      typeof matrix[r][c] === "number" &&
      abs(matrix[r][c]) >= abs(matrix[or][oc]))
  ) {
