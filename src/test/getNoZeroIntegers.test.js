...
describe('引数の値を与えてゼロなしの整数の合計の配列にして返却する', () => {
    ...
    test('11の引数を渡して[2,9]が返却される', () => {
        expect(getNoZeroIntegers(11))
            .toEqual(expect.arrayContaining([2, 9]));
    });

    test('10000の引数を渡して[1,9999]が返却される', () => {
        expect(getNoZeroIntegers(10000))
            .toEqual(expect.arrayContaining([1, 9999]));
    });

    test('69の引数を渡して[1, 68]が返却される', () => {
        expect(getNoZeroIntegers(69))
            .toEqual(expect.arrayContaining([1, 68]));
    });

    test('1010の引数を渡して[11,999]が返却される', () => {
        expect(getNoZeroIntegers(1010))
            .toEqual(expect.arrayContaining([11, 999]));
    });
});