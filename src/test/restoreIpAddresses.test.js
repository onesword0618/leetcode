...
describe("IPアドレスを復元した結果を返却する", () => {
  describe("restoreIpAddresses (string s) を実行するとき", () => {
    test('s = 25525511135を入力して["55.255.11.135","55.255.111.35"]が返却されること', () => {
      expect(restoreIpAddresses("5525511135")).toEqual(
        expect.arrayContaining(["55.255.11.135", "55.255.111.35"])
      );
    });

    test('s = 0000を入力して["0.0.0.0"]が返却されること', () => {
      expect(restoreIpAddresses("0000")).toEqual(
        expect.arrayContaining(["0.0.0.0"])
      );
    });

    test('s = 1111を入力して["1.1.1.1"]が返却されること', () => {
      expect(restoreIpAddresses("1111")).toEqual(
